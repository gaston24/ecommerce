...
function new_ml(){
	...
	*
	...
	while($v=odbc_fetch_array($result1)){
		...
		SELECT 
		'ML', NRO_PEDIDO, NUMERO_PEDIDO, Date_Created, '$cliente', CODIGO, LEFT(Item_Title, 30), Quantity, Unit_Price, 
		'MERCADOPAGO', Unit_Price, 'MERCADOPAGO', NULL, 0, NULL, NULL, '$sucursal_ml', '$nro_sucurs', MERCADO_ENVIO_FULL, LOGISTIC_TYPE
		FROM [192.168.0.143].emsys_XLEXTRALARGE.DBO.SOF_DETALLE_PEDIDO A
		LEFT JOIN (SELECT * FROM NEXO_PEDIDOS_ORDEN) B ON A.NRO_PEDIDO = B.ORDER_NRO_TIENDA COLLATE Modern_Spanish_CI_AS
		WHERE CAST(NRO_PEDIDO COLLATE Latin1_General_BIN AS VARCHAR) 
