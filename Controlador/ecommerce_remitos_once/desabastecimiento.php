...
function insertar($remito){
	...
	FROM STA14 
	WHERE COD_PRO_CL = 'GTCENT' AND N_COMP = '$remito'
	...
	SELECT  
	[FILLER], [TCOMP_IN_S], [NCOMP_IN_S], 1, 1, 0, 0, [CANTIDAD], 0, [CANTIDAD], [CANTIDAD], 0, [COD_ARTICU], '09', 
	...
	SELECT '', B.N_PARTIDA, A.NCOMP_IN_S, A.TCOMP_IN_S, A.CANTIDAD, A.CANTIDAD, A.COD_ARTICU, '09', '01', [N_RENGL_S], 1, 'E', 0, 0, 0, 0
	FROM STA20 A
	...
	UPDATE STA14 SET EXPORTADO = 1 WHERE COD_PRO_CL = 'GTCENT' AND N_COMP = '$remito'
	
	UPDATE CTA96 SET COD_DEPENT = '01'
	WHERE TCOMP_IN_S = 'RE' AND NCOMP_IN_S IN
	(SELECT NCOMP_IN_S FROM STA14 WHERE COD_PRO_CL = 'GTCENT' )
